...
class AwardController extends Controller
{
     ...
     * )
     */
    public function store(StoreAwardRequest $request, CreateAwardAction $createAward): JsonResponse
    {
        ...
        $award = $createAward->execute(
            CreateAwardDTO::fromRequest($request, $architectId, $verificationFilePath)
        );
     ...
     * )
     */
    public function update(UpdateAwardRequest $request, string $id, UpdateAwardAction $updateAward): JsonResponse
    {
        ...
        if ($isAdmin) {
            ...
            $award = $updateAward->execute(
                $award,
                UpdateAwardDTO::fromArray(['status' => $data['status']])
            );
        ...
        if ($request->hasFile('verification_file')) {
            ...
            $verificationFilePath = $request->file('verification_file')->store('awards/verification-files', 'public');
        }

        $award = $updateAward->execute(
            $award,
            UpdateAwardDTO::fromRequest($request, $verificationFilePath)->withoutStatus(),
            resetStatus: true,
        );
