...
<div class="max-w-7xl mx-auto space-y-6 pb-12">
    ...
    <div class="flex items-center justify-between">
        <a href="{{ route('admin.dashboard.index') }}" class="inline-flex items-center text-slate-800 hover:text-[#E8820C] transition-colors">
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-[#FDFBF7] text-slate-600 text-[11px] font-bold uppercase tracking-wider">
                    ...
                    <th class="py-4 px-6">Award Status</th>
                    <th class="py-4 px-6 text-center">Proof</th>
                    <th class="py-4 px-6 text-center">Actions</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-50" id="awards-table-body">
                <tr>
                    <td colspan="7" class="px-6 py-8 text-center text-slate-400 font-medium">Loading awards data...</td>
                </tr>
            </tbody>
        </table>
...
<script>
    ...
    async function fetchAwards(page = 1) {
        ...
        let url = @js(route('admin.dashboard.architects.awards')) + `?page=${page}&per_page=10`;
    ...
    async function fetchStatistics() {
        try {
            const response = await fetch(@js(route('admin.dashboard.architects.stats')), {
                headers: {
                    'Accept': 'application/json'
                }
            });
